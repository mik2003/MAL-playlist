...
export function createPlaylistItem(shuffledIndex, displayIndex) {
    const currentSongData = playerState.animePlaylist[shuffledIndex];
...
export function createEpisodeContainer(type, songData) {
...
export function getAvailableSources() {
    const currentSong = playerState.animePlaylist[playerState.playlistIndeces[playerState.currentIndex]];
    const sources = [];
    if (currentSong.yt_url)
        sources.push('yt_url');
    if (currentSong.at_url)
        sources.push('at_url');
    return sources;
}
